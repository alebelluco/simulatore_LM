...
with tab_input:
    ...
    with sx1:
        codici_1 = db[(db.Macchina == 'DMG4')].Particolare.unique()
        codici_1 = np.append(codici_1, 'Macchina Spenta')
        pn1 = st.selectbox("Selezionare il codice per DMG4", options=codici_1)
        versioni1 = db[(db.Particolare == pn1) & (db.Macchina == 'DMG4')].Versione.astype(str).unique()
        ver1 = versioni1[0]
        if len(versioni1) != 1:
        ...
        chiavi.append(f'1ADDMG4{pn1}{ver1}')

    with cx1: 
        codici_2 = db[(db.Macchina == 'DMG5') ].Particolare.unique()
        pn2 = st.selectbox("Selezionare il codice per DMG5", options=codici_2)
        versioni2 = db[(db.Particolare == pn2) & (db.Macchina == 'DMG5')].Versione.astype(str).unique()
        ver2 = versioni2[0]
        if len(versioni2) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver2 = st.selectbox('Selezionare la versione desiderata', options=versioni2, key=2)
        chiavi.append(f'1ADDMG5{pn2}{ver2}')

    with dx1:   
        codici_3 = db[(db.Macchina == 'DMG6') ].Particolare.unique()
        pn3 = st.selectbox("Selezionare il codice per DMG6", options=codici_3)
        versioni3 = db[(db.Particolare == pn3) & (db.Macchina == 'DMG6')].Versione.astype(str).unique()
        ver3 = versioni3[0]
        if len(versioni3) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver3 = st.selectbox('Selezionare la versione desiderata', options=versioni3, key=3)
        chiavi.append(f'1ADDMG6{pn3}{ver3}')

    with dxx1:   
        codici_4 = db[(db.Macchina == 'DMG7') ].Particolare.unique()
        pn4 = st.selectbox("Selezionare il codice per DMG7", options=codici_4)
        versioni4 = db[(db.Particolare == pn4) & (db.Macchina == 'DMG7')].Versione.astype(str).unique()
        ver4 = versioni4[0]
        if len(versioni4) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver4 = st.selectbox('Selezionare la versione desiderata', options=versioni4, key=4)
        chiavi.append(f'1ADDMG7{pn4}{ver4}')
