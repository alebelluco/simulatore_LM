...
with tab_input:
    ...
    # Subset del database: filtro sull'isola
    isole = [isola]
    ...
    with sel1:
        st.subheader(m1)
        codici_1 = db[(db.Isola == isola) & (db.Macchina == m1) ].Particolare.unique()
        pn1 = st.selectbox(f"Selezionare il codice per {m1}", options=codici_1)
        versioni1 = db[(db.Isola ==isola) & (db.Particolare == pn1)].Versione.astype(str).unique()
        ver1 = versioni1[0]
        if len(versioni1) != 1:
        ...
        chiavi.append(f'{isola}{m1}{pn1}{ver1}')
    with sel2: 
        st.subheader(m2)
        codici_2 = db[(db.Isola == isola) & (db.Macchina == m2) ].Particolare.unique()
        pn2 = st.selectbox(f"Selezionare il codice per {m2}", options=codici_2)
        versioni2 = db[(db.Isola ==isola) & (db.Particolare == pn2)].Versione.astype(str).unique()
        ver2 = versioni2[0]
        if len(versioni2) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver2 = st.selectbox('Selezionare la versione desiderata', options=versioni2, key='2')
        chiavi.append(f'{isola}{m2}{pn2}{ver2}')
    with sel3:   
        st.subheader(m3)
        codici_3 = db[(db.Isola == isola) & (db.Macchina == m3) ].Particolare.unique()
        pn3 = st.selectbox(f"Selezionare il codice per {m3}", options=codici_3)
        versioni3 = db[(db.Isola ==isola) & (db.Particolare == pn3)].Versione.astype(str).unique()
        ver3 = versioni3[0]
        if len(versioni3) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver3 = st.selectbox('Selezionare la versione desiderata', options=versioni3, key='3')
        chiavi.append(f'{isola}{m3}{pn3}{ver3}')
    with sel4:   
        st.subheader(m4)
        codici_4 = db[(db.Isola == isola) & (db.Macchina == m4) ].Particolare.unique()
        pn4 = st.selectbox(f"Selezionare il codice per {m4}", options=codici_4)
        versioni4 = db[(db.Isola ==isola) & (db.Particolare == pn4)].Versione.astype(str).unique()
        ver4 = versioni4[0]
        if len(versioni4) != 1:
            st.warning('È presente più di una versione di ciclo')
            ver4 = st.selectbox('Selezionare la versione desiderata', options=versioni4, key='4')
        chiavi.append(f'{isola}{m4}{pn4}{ver4}')
